...
format_r_vector <- function(values) {
  ...
  if (is.logical(values)) {
    formatted <- ifelse(values, "TRUE", "FALSE")
    return(paste0("c(", paste(formatted, collapse = ", "), ")"))
  }

  escaped <- vapply(as.character(values), escape_r_string, character(1))
  paste0("c(", paste(escaped, collapse = ", "), ")")
}

build_factor_level_script_lines <- function(applied_factor_levels) {
  if (is.null(applied_factor_levels) || length(applied_factor_levels) == 0) {
    return(character(0))
  }

  lines <- c("", "# Reapply factor level edits from Dendro")
  for (var_name in names(applied_factor_levels)) {
    levels_code <- format_r_vector(applied_factor_levels[[var_name]])
    lines <- c(
      lines,
      paste0("data_raw$", var_name, " <- factor(data_raw$", var_name, ", levels = ", levels_code, ")")
    )
  }
...
build_filter_script_lines <- function(active_filter) {
  if (is.null(active_filter) || is.null(active_filter$variable)) {
    return(c("active_filter <- NULL"))
  }

  if (identical(active_filter$type, "numeric")) {
    filter_code <- c(
      "active_filter <- list(",
      paste0("  variable = ", escape_r_string(active_filter$variable), ","),
      "  type = \"numeric\",",
      paste0("  range = ", format_r_vector(active_filter$range)),
      ")"
    )
  } else {
    filter_code <- c(
      ...
      paste0("  variable = ", escape_r_string(active_filter$variable), ","),
      "  type = \"categorical\",",
      paste0("  values = ", format_r_vector(active_filter$values)),
      ")"
    )
  }
...
build_data_load_script_lines <- function(data_source) {
  if (!is.null(data_source) && identical(data_source$type, "demo") && !is.null(data_source$demo_id)) {
    return(c(
      "source(\"R/utils_demo_data.R\")",
      "",
      paste0("demo <- generate_demo_dataset(", escape_r_string(data_source$demo_id), ")"),
      "data_raw <- demo$data"
    ))
  }

  original_name <- if (!is.null(data_source) && !is.null(data_source$filename)) {
  ...
  c(
    "# Replace this with the real path to your source dataset",
    paste0("data_path <- ", escape_r_string(original_name)),
    "data_raw <- load_data_file(data_path, basename(data_path))"
  )
}

generate_analysis_script <- function(data_source, active_filter, outcome_var, predictors,
                                     cp, minbucket, maxdepth, applied_factor_levels = list()) {
  script_lines <- c(
    "# Generated by Dendro",
    ...
    "",
    "# Load the dataset",
    build_data_load_script_lines(data_source),
    build_factor_level_script_lines(applied_factor_levels),
    "",
    "# Analysis settings",
    paste0("outcome_var <- ", escape_r_string(outcome_var)),
    paste0("predictors <- ", format_r_vector(predictors)),
    paste0("cp <- ", format(cp, trim = TRUE, scientific = FALSE)),
    paste0("minbucket <- ", format(minbucket, trim = TRUE, scientific = FALSE)),
    paste0("maxdepth <- ", format(maxdepth, trim = TRUE, scientific = FALSE)),
    "",
    "# Active filter",
    build_filter_script_lines(active_filter),
    "data_filtered <- apply_single_filter(data_raw, active_filter)",
    "",
    "if (nrow(data_filtered) == 0) {",
    "  stop(\"The active filter removed all rows. Widen the filter or clear it.\")",
    "}",
    "",
    "constant_predictors <- get_constant_predictors(data_filtered, predictors)",
    "if (length(constant_predictors) > 0) {",
    "  message(\"Dropping constant predictors: \", paste(constant_predictors, collapse = \", \"))",
    "}",
    "predictors <- setdiff(predictors, constant_predictors)",
    "",
    "if (length(predictors) == 0) {",
    "  stop(\"No usable predictors remain after filtering.\")",
    "}",
    "",
    "outcome_values <- unique(stats::na.omit(data_filtered[[outcome_var]]))",
    "if (length(outcome_values) < 2) {",
    "  stop(\"Outcome has fewer than two values after filtering.\")",
    "}",
    "",
    "# Fit the model",
    ...
    "",
    "print(model)",
    "render_tree_plot(model, title = paste(\"Decision Tree:\", outcome_var))",
    "",
    "# Optional: print rules to the console",
    "rules <- capture.output(rpart.plot::rpart.rules(model, style = \"tall\", cover = TRUE, roundint = FALSE))",
...
create_r_script_handler <- function(rv) {
  downloadHandler(
    filename = function() {
      paste0("decision_tree_analysis_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".R")
    },
    content = function(file) {
      req(rv$model, rv$data, rv$outcome_var, rv$predictor_vars)

      script_text <- generate_analysis_script(
        ...
        applied_factor_levels = rv$applied_factor_levels
      )
